-- Open parent directory of current file with oil
...
-- Show lsp hover on ctrl + K and shift + K default
...
-- LSP based fzf keymaps
vim.api.nvim_create_autocmd("LspAttach", {
    group = vim.api.nvim_create_augroup("keymaps-lsp-atach", { clear = true }),
    callback = function()

        -- [G]oto [D]efinition(s)
        vim.keymap.set("n", "gd", function()
            local params = vim.lsp.util.make_position_params()
            vim.lsp.buf_request(0, "textDocument/definitions", params, function(_, result)
                local item = result
        ...
        vim.keymap.set("n", "gI", function()
            ...
            vim.lsp.buf_request(0, "textDocument/implementation", params, function(_, result)
                ...
                if not items or vim.tbl_isempty(items) then
                    vim.notify("No implementation found", vim.log.level.ERROR)
                elseif #items == 1 then
